#!/bin/bash
set -e
if [ "x$JAVA_HOME19" == "x" ] ; then
    echo 'Must specify $JAVA_HOME19, pointing to JDK 19 home'
    exit 1;
fi;
...
JAVA_HOME=$JAVA_HOME19 PATH=$JAVA_HOME/bin:$PATH javac -d build/tools/classes -source 19 -target 19 --limit-modules java.base,java.xml,jdk.zipfs --add-exports java.base/jdk.internal=ALL-UNNAMED `find openjdk/make/langtools/tools/propertiesparser -name "*.java" -type f` || exit 1
...
JAVA_HOME=$JAVA_HOME19 PATH=$JAVA_HOME/bin:$PATH java -cp build/tools/classes propertiesparser.PropertiesParser -compile openjdk/src/jdk.compiler/share/classes/com/sun/tools/javac/resources/compiler.properties build/tools/properties || exit 1
...
(cd build/ct.sym-source;
 $JAVA_HOME19/bin/java --add-exports jdk.jdeps/com.sun.tools.classfile=ALL-UNNAMED \
...
JAVA_HOME=$JAVA_HOME19 PATH=$JAVA_HOME/bin:$PATH javac -d build/phase1/classes -source 19 -target 19 --limit-modules java.base,java.xml,jdk.zipfs $SOURCEFILES || exit 1
...
JAVA_HOME=$JAVA_HOME19 PATH=$JAVA_HOME/bin:$PATH java --limit-modules java.base,java.xml,jdk.zipfs -classpath build/phase1/classes org.frgaal.Main -source 19 -target 8 --limit-modules java.base -d build/phase2/classes $SOURCEFILES || exit 1
...
JAVA_HOME=$JAVA_HOME8 PATH=$JAVA_HOME/bin:$PATH java -classpath build/phase2/classes org.frgaal.Main -source 19 -target 8 --limit-modules java.base -d build/phase3/classes $SOURCEFILES || exit 1
mkdir -p build/phase3/classes/META-INF/versions/9
JAVA_HOME=$JAVA_HOME8 PATH=$JAVA_HOME/bin:$PATH java -classpath build/phase2/classes org.frgaal.Main -source 19 -target 9 --limit-modules java.base -d build/phase3/classes/META-INF/versions/9 openjdk/src/java.compiler/share/classes/javax/lang/model/SourceVersion.java || exit 1
...
EOF
(cd build/phase3/classes; $JAVA_HOME19/bin/jar -J-Xmx2048m --create --manifest=../manifest.mf -f ../../../dist/compiler/compiler-`cat ../../../VERSION`.jar `find . -type f`)
...
(cd build/compiler/sources; $JAVA_HOME19/bin/jar --create -f ../../../dist/compiler/compiler-`cat ../../../VERSION`-sources.jar `find . -type f`)

mkdir -p build/compiler/javadoc
echo "No javadoc" >build/compiler/javadoc/README

(cd build/compiler/javadoc; $JAVA_HOME19/bin/jar --create -f ../../../dist/compiler/compiler-`cat ../../../VERSION`-javadoc.jar `find . -type f`)
