...
JAVA_HOME=$JAVA_HOME21 PATH=$JAVA_HOME/bin:$PATH javac -d build/tools/classes -source 21 -target 21 --limit-modules java.base,java.xml,jdk.zipfs --add-exports java.base/jdk.internal=ALL-UNNAMED `find openjdk/make/langtools/tools/propertiesparser -name "*.java" -type f` || exit 1
...
JAVA_HOME=$JAVA_HOME21 PATH=$JAVA_HOME/bin:$PATH java -cp build/tools/classes propertiesparser.PropertiesParser -compile openjdk/src/jdk.compiler/share/classes/com/sun/tools/javac/resources/compiler.properties build/tools/properties || exit 1
...
(cd build/ct.sym-source;
 $JAVA_HOME21/bin/java --add-exports jdk.jdeps/com.sun.tools.classfile=ALL-UNNAMED \
...
JAVA_HOME=$JAVA_HOME21 PATH=$JAVA_HOME/bin:$PATH javac -d build/phase1/classes --release 20 --limit-modules java.base,java.xml,jdk.zipfs $SOURCEFILES || exit 1
...
JAVA_HOME=$JAVA_HOME21 PATH=$JAVA_HOME/bin:$PATH java --limit-modules java.base,java.xml,jdk.zipfs -classpath build/phase1/classes org.frgaal.Main -source 20 -target 8 --limit-modules java.base -d build/phase2/classes $SOURCEFILES || exit 1
...
EOF
(cd build/phase3/classes; $JAVA_HOME21/bin/jar -J-Xmx2048m --create --manifest=../manifest.mf -f ../../../dist/compiler/compiler-`cat ../../../VERSION`.jar `find . -type f`)
...
(cd build/compiler/sources; $JAVA_HOME21/bin/jar --create -f ../../../dist/compiler/compiler-`cat ../../../VERSION`-sources.jar `find . -type f`)

mkdir -p build/compiler/javadoc
echo "No javadoc" >build/compiler/javadoc/README

(cd build/compiler/javadoc; $JAVA_HOME21/bin/jar --create -f ../../../dist/compiler/compiler-`cat ../../../VERSION`-javadoc.jar `find . -type f`)
