#!/bin/bash
set -e
if [ "x$JAVA_HOME17" == "x" ] ; then
    echo 'Must specify $JAVA_HOME17, pointing to JDK 17 home'
    exit 1;
fi;
...
JAVA_HOME=$JAVA_HOME17 PATH=$JAVA_HOME/bin:$PATH javac -d build/tools/classes -source 17 -target 17 --limit-modules java.base,java.xml,jdk.zipfs --add-exports java.base/jdk.internal=ALL-UNNAMED `find openjdk/make/langtools/tools/propertiesparser -name "*.java" -type f` || exit 1
...
(cd build/ct.sym-source;
 $JAVA_HOME17/bin/java --add-exports jdk.jdeps/com.sun.tools.classfile=ALL-UNNAMED \
...
JAVA_HOME=$JAVA_HOME17 PATH=$JAVA_HOME/bin:$PATH javac -d build/phase1/classes -source 17 -target 17 --limit-modules java.base,java.xml,jdk.zipfs $SOURCEFILES || exit 1
...
JAVA_HOME=$JAVA_HOME17 PATH=$JAVA_HOME/bin:$PATH java --limit-modules java.base,java.xml,jdk.zipfs -classpath build/phase1/classes org.frgaal.Main -source 17 -target 8 -d build/phase2/classes $SOURCEFILES || exit 1
...
JAVA_HOME=$JAVA_HOME8 PATH=$JAVA_HOME/bin:$PATH java -classpath build/phase2/classes org.frgaal.Main -source 17 -target 8 -d build/phase3/classes $SOURCEFILES || exit 1
...
(cd build/phase3/classes; $JAVA_HOME17/bin/jar -J-Xmx2048m --create --main-class org.frgaal.Main -f ../../../dist/compiler/compiler-`cat ../../../VERSION`.jar `find . -type f`)
...
(cd build/compiler/sources; $JAVA_HOME17/bin/jar --create -f ../../../dist/compiler/compiler-`cat ../../../VERSION`-sources.jar `find . -type f`)

mkdir -p build/compiler/javadoc
echo "No javadoc" >build/compiler/javadoc/README

(cd build/compiler/javadoc; $JAVA_HOME17/bin/jar --create -f ../../../dist/compiler/compiler-`cat ../../../VERSION`-javadoc.jar `find . -type f`)
