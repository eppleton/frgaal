...
$JAVA_HOME8/bin/java -jar dist/compiler/compiler-`cat VERSION`.jar -d build/javadoc/classes -Xbootclasspath/p:dist/compiler/compiler-`cat VERSION`.jar -classpath dist/compiler/compiler-`cat VERSION`.jar $JAVADOC_SOURCEFILES
...
(cd build/javadoc/classes; $JAVA_HOME18/bin/jar -J-Xmx2048m --create -m ../manifest.mf --main-class org.frgaal.javadoc.Main -f ../../../dist/javadoc/javadoc-`cat ../../../VERSION`.jar `find . -type f`)

rm -rf build/javadoc/sources
mkdir -p build/javadoc/sources
cp -r openjdk/src/jdk.javadoc/share/classes/* build/javadoc/sources

(cd build/javadoc/sources; $JAVA_HOME18/bin/jar --create -f ../../../dist/javadoc/javadoc-`cat ../../../VERSION`-sources.jar `find . -type f`)

mkdir -p build/javadoc/javadoc
echo "No javadoc" >build/javadoc/javadoc/README

(cd build/javadoc/javadoc; $JAVA_HOME18/bin/jar --create -f ../../../dist/javadoc/javadoc-`cat ../../../VERSION`-javadoc.jar `find . -type f`)
