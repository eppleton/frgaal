...
$JAVA_HOME18/bin/java -jar $COMPILER -target 8 -d $TARGET/test -classpath $TARGET/main -processor org.frgaal.tests.sourceversion.api.AP `find src/test/java -type f` >$TARGET/output18

cat >$TARGET/expected18 <<EOF
\$values[]
getLatestSupported[]
isIdentifier[interface java.lang.CharSequence]
isKeyword[interface java.lang.CharSequence, class javax.lang.model.SourceVersion]
...
if [ "x`diff $TARGET/expected18 $TARGET/output18 | tee $TARGET/diff`" != "x" ] ; then
    echo "Incorrect output!";
    exit 1
fi
