...
public class BasicRecordTest extends TestBase {
    ...
    public void testCanCompileRecordToJDK8() throws Exception {
        ...
        int compilationResult = org.frgaal.Main.compile(new String[] {"-d", classDir.toString(), "-source", "19", "-target", "8", recordSource.toString(), extraSource.toString()}, pout);
        pout.close();
    ...
    public void testNoTargetDir() throws Exception {
        ...
        StringWriter out = new StringWriter();
        PrintWriter pout = new PrintWriter(out);
        int compilationResult = org.frgaal.Main.compile(new String[] {"-XDrawDiagnostics", "-source", "19", "-target", "8", recordSource.toString()}, pout);
        pout.close();
        assertEquals(out.toString(), 0, compilationResult);
        assertEquals("""
    ...
    public void testSerializableRecords() throws Exception {
        ...
        StringWriter out = new StringWriter();
        PrintWriter pout = new PrintWriter(out);
        int compilationResult = org.frgaal.Main.compile(new String[] {"-d", classDir.toString(), "-XDrawDiagnostics", "-source", "19", "-target", "8", recordSource.toString()}, pout);
        pout.close();
