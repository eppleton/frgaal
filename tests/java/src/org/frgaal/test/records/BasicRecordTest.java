...
public class BasicRecordTest extends TestBase {
    ...
    public void testSeparateCompilation() throws Exception {
        {
            Path recordSource = writeFile("R.java",
                                          """
                                          package p;
        ...
        {
            Path testSource = writeFile("Test.java",
                                        """
                                        package p;
                                        public class Test {
                                            private void test(Object o) {
                                                boolean b = o instanceof R(var v);
                                            }
                                        }
                                        """);
            StringWriter out = new StringWriter();
            PrintWriter pout = new PrintWriter(out);
            int compilationResult = org.frgaal.Main.compile(new String[] {"-d", classDir.toString(), "-classpath", classDir.toString(), "--enable-preview", "-source", "20", "-XDrawDiagnostics", "-target", "8", testSource.toString()}, pout);
            pout.close();
            assertEquals(out.toString(), 0, compilationResult);
        }
    }
