#!/bin/bash
if [ "x$JAVA_HOME15" == "x" ] ; then
    echo 'Must specify $JAVA_HOME15, pointing to JDK 15 home'
    exit 1;
fi;
...
JAVA_HOME=$JAVA_HOME15 PATH=$JAVA_HOME/bin:$PATH javac -d build/tools/classes -source 15 -target 15 --limit-modules java.base,java.xml,jdk.zipfs --add-exports java.base/jdk.internal=ALL-UNNAMED `find openjdk/make/langtools/tools/propertiesparser -name "*.java" -type f` || exit 1
...
JAVA_HOME=$JAVA_HOME15 PATH=$JAVA_HOME/bin:$PATH java -cp build/tools/classes propertiesparser.PropertiesParser -compile openjdk/src/jdk.compiler/share/classes/com/sun/tools/javac/resources/compiler.properties build/tools/properties || exit 1
...
(cd build/ct.sym-source;
 $JAVA_HOME15/bin/java --add-exports jdk.jdeps/com.sun.tools.classfile=ALL-UNNAMED \
...
JAVA_HOME=$JAVA_HOME15 PATH=$JAVA_HOME/bin:$PATH javac -d build/phase1/classes -source 15 -target 15 --limit-modules java.base,java.xml,jdk.zipfs $SOURCEFILES || exit 1
...
JAVA_HOME=$JAVA_HOME15 PATH=$JAVA_HOME/bin:$PATH java --limit-modules java.base,java.xml,jdk.zipfs -classpath build/phase1/classes org.frgaal.Main -source 15 -target 8 -d build/phase2/classes $SOURCEFILES || exit 1
...
#JAVA_HOME=$JAVA_HOME8 PATH=$JAVA_HOME/bin:$PATH java -classpath build/phase2/classes org.frgaal.Main -source 15 -target 8 -bootclasspath $JAVA_HOME8/jre/lib/rt.jar -d build/phase3/classes $SOURCEFILES || exit 1
JAVA_HOME=$JAVA_HOME8 PATH=$JAVA_HOME/bin:$PATH java -classpath build/phase2/classes org.frgaal.Main -source 15 -target 8 -d build/phase3/classes $SOURCEFILES || exit 1
...
(cd build/phase3/classes; $JAVA_HOME15/bin/jar -J-Xmx2048m --create --main-class org.frgaal.Main -f ../../../dist/compiler-`cat ../../../VERSION`.jar `find . -type f`)
...
(cd build/sources; $JAVA_HOME15/bin/jar --create -f ../../dist/compiler-`cat ../../VERSION`-sources.jar `find . -type f`)

mkdir -p build/javadoc
echo "No javadoc" >build/javadoc/README

(cd build/javadoc; $JAVA_HOME15/bin/jar --create -f ../../dist/compiler-`cat ../../VERSION`-javadoc.jar `find . -type f`)
